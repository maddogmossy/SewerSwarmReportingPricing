...
/* ---------- per-sector styling (matches P2 cards) ---------- */
...
/* ---------- file filters for the native dialog ---------- */
const ACCEPT =
  'application/pdf,.pdf,application/x-sqlite3,application/vnd.sqlite3,application/octet-stream,.db,.db3';

/* ---------- helpers ---------- */
const isPdf  = (n: string) => n.toLowerCase().endsWith('.pdf');
const isDb   = (n: string) => /\.db3?$/i.test(n);
const isMeta = (n: string) => /_meta\.db3?$/i.test(n);
/* compare the base names case-insensitively and ignore the “_Meta” */
const baseDb = (n: string) =>
  n.replace(/_meta(?=\.db3?$)/i, '').toLowerCase();

/* Single PDF or a DB pair (main+_Meta) */
...
export default function UploadClient() {
  ...
  /** Deduplicate by name+size+mtime */
  ...
  /** Always open the NORMAL FILE dialog (not the directory picker). */
  ...
  return (
    <div className={`rounded-xl border ${style.border} bg-white shadow-sm`}>
      {/* Header styled like P2 sector card + P3 page badge */}
      <div className={`flex items-center gap-3 border-b ${style.border} px-4 py-3`}>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center rounded-full bg-neutral-900 px-2 py-0.5 text-xs font-semibold text-white">P3</span>
      ...
      <div className="p-4">
        ...
        >
          ...
          <div className="text-sm">
            ...
            <button
              ...
              title="Open the standard file picker"
            >
              browse
            </button>
          </div>
        ...
        {/* Hidden input that opens the NORMAL file picker */}
        <input
          ref={fileInputRef}
          type="file"
          multiple
