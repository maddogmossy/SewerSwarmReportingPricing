...
//
// Requirements:
// - @vercel/blob installed (package.json): "@vercel/blob": "^0.24.0"
// - Vercel env var BLOB_READ_WRITE_TOKEN with RW scope for your store
//
// This page:
//  • Title/subtitle (as requested)
//  • Lists Clients ➜ Projects ➜ Files by parsing blob pathname
//  • Delete controls for: file / project / client
//  • Revalidates the page after each action

import { list, del } from '@vercel/blob';
import { revalidatePath } from 'next/cache';
import { Folder, FileText, Database, Trash2, ChevronRight, ChevronDown } from 'lucide-react';

// Optional: keep a consistent badge like other pages
function PageBadge() {
  return (
...
function groupByClientProject(blobs: FileLeaf[]) {
  ...
  for (const b of blobs) {
    ...
    if (parts.length < 4) continue; // skip anything not matching the scheme
    // const [sector, client, project, filename] = parts;
...
export async function deleteProjectAction(formData: FormData) {
  ...
  const prefix = `${sector}/${client}/${project}/`; // trailing slash!
  // delete all blobs under this project prefix
...
export async function deleteClientAction(formData: FormData) {
  ...
  const prefix = `${sector}/${client}/`; // trailing slash!
  let cursor: string | undefined = undefined;
  do {
    const { blobs, cursor: next } = await list({ prefix, cursor });
    if (blobs?.length) {
...
export default async function ReportsPage() {
  // Pull all blobs for all sectors; if your store is large,
  // replace with multiple list({ prefix: 'S1/' }), etc.
  const all: FileLeaf[] = [];
  let cursor: string | undefined = undefined;
  do {
    const { blobs, cursor: next } = await list({ cursor });
    for (const b of blobs) {
      // b.pathname includes the path inside the blob store
      all.push({
        url: b.url,
        pathname: b.pathname,
        size: b.size,
  ...
  // Build Sector ➜ Client ➜ Project ➜ Files
  // We’ll keep sectors as top-level sections.
  const bySector = new Map<string, FileLeaf[]>();
  for (const f of all) {
    const parts = f.pathname.split('/').filter(Boolean);
    if (parts.length < 4) continue; // expect S#/Client/Project/file
                  ...
                  return (
                    <details key={client} className="rounded-lg border border-slate-200">
                      <summary className="flex cursor-pointer list-none items-center gap-2 bg-slate-50 px-3 py-2">
                        ...
                        {/* Delete client (all projects & files) */}
