...
330 - August 1, 2025. CRITICAL F615 DASHBOARD CALCULATION FIX AND MM1 AUTO-SELECTION: Fixed major F615 dashboard logic error where system incorrectly read blue window (day rate £1650) as cost per patch instead of green window pricing. Corrected F615 workflow understanding: Blue=Day Rate, Green=Patch Prices (£450 Row 2 default), Purple=Minimum quantities. Updated dashboard cost calculation to use greenValue field for cost per patch (£450) instead of blueValue field (£1650). Enhanced F615 structural defect workflow to access purple window patchingGreenData for minimum quantity validation. Added MM1 utilities (id1) card auto-selection functionality when dashboard routes to F615 patching configuration with autoSelectUtilities=true parameter. System now properly calculates £450 × defect count for structural patching instead of incorrect £165 calculation. Complete F615 pricing workflow operational with correct MM1 sector selection.
