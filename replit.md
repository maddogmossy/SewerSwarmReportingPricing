...
- **Report Reprocessing System Investigation (Jan 2025)**: Completed comprehensive analysis of data processing pipeline. System has active reprocessing capability through `/api/upload` endpoint with automatic sector detection and complete data clearing. Identified critical pipe size bug where sections 1-5,17 show 100mm instead of correct 150mm despite database containing accurate 150mm values. Created detailed Instructions.md with systematic approach for ensuring uniform report processing. Root cause: pipe size conversion error in processing pipeline, not database extraction issue.
