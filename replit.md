...
- June 28, 2025. Implemented intuitive clickable column header system with localStorage persistence: Replaced checkbox panel with direct column header clicking for hiding columns. Users click "Hide Columns" button then click any optional column header to hide it - headers highlight red on hover and show tooltips. Essential columns (Item No, MH references, Defects, Cost) are protected and display "(required)" labels. Column preferences persist across sessions via localStorage. Enhanced recommendations to be user-friendly without grade references - "We recommend cleansing and resurvey due to debris" for Section 3, with specific recommendations for different defect types.
```
