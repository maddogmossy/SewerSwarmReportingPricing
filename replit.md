...
- Defect Text Standardization: Standardized Grade 0 defect text across processing pipeline from "No defects recorded" to "No service or structural defect found" for consistency.
...
- A5/A4 Equipment Selection Bug Fix: Resolved critical UI mismatch where A5 selection displayed as A4 in recommendation interface. Root cause was dynamic array reordering in equipmentOptions that caused configId display confusion. Fixed by implementing static equipment options with consistent configId mapping (A5=id760, A4=id759) and replaced isDefault with isPriority for accurate state reflection. Simplified priority button logic by removing complex buffer manipulation that caused race conditions. System now correctly displays A5 when A5 is selected and A4 when A4 is selected, eliminating user confusion about equipment priority state.
- Dashboard Navigation Fix: Resolved issue where A4/A5 equipment selection didn't return users to dashboard. Root cause was missing setIsOpen(false) calls in simplified priority button handlers. Added dialog close functionality to both A4 and A5 button click handlers, restoring expected navigation behavior where users return to dashboard after equipment priority selection.
- Equipment Priority State Synchronization Fix: Resolved critical inconsistency where different dashboard items showed different equipment priorities (Item 21 showing A4 while Item 3 showing A5 despite A5 being selected). Root cause was mixed usage of component state vs localStorage direct reads. Eliminated all direct localStorage.getItem('equipmentPriority') calls in favor of unified component state usage. Refactored cost decision restoration from useState initialization to useEffect with equipmentPriority dependency. All dashboard items now consistently use the same equipment priority source, ensuring uniform A5/A4 display across all items.
