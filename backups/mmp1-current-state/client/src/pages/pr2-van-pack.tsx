...
import { ChevronLeft, Save, Calculator, Truck } from 'lucide-react';

export default function PR2VanPack() {
  ...
  // Form state specific to Van Pack category
  const [formData, setFormData] = useState({
    categoryName: 'Van Pack',
    description: 'Mobile van-based equipment package',
    pricingOptions: {
  ...
  const saveConfiguration = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: `PR2 Van Pack configuration ${isEditing ? 'updated' : 'created'} successfully` });
  ...
  if (configLoading) {
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">Loading Van Pack configuration...</div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Van Pack Configuration</h1>
              <p className="text-gray-600">Configure pricing for mobile van-based equipment package</p>
            </div>
          </div>
        </div>
      </div>
