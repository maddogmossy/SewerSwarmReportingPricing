...
import { ChevronLeft, Save, Calculator, Video } from 'lucide-react';

export default function PR2CCTV() {
  ...
  // Form state specific to CCTV category
  const [formData, setFormData] = useState({
    categoryName: 'CCTV',
    description: 'Closed-circuit television inspection services',
    pricingOptions: {
  ...
  const saveConfiguration = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: `PR2 CCTV configuration ${isEditing ? 'updated' : 'created'} successfully` });
  ...
  if (configLoading) {
    return (
      <div className="container mx-auto p-6">
        <div className="text-center">Loading CCTV configuration...</div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">CCTV Configuration</h1>
              <p className="text-gray-600">Configure pricing for CCTV inspection services</p>
            </div>
          </div>
        </div>
      </div>
