...
export default function RepairPricing() {
  ...
  // Removed old pricing dialog state
  ...
  // Removed old formData state - now using PR1 pricing system
  ...
  useEffect(() => {
    ...
    if (editMode === 'true' && editId && pricingData) {
      ...
      if (pricingToEdit) {
        ...
        // Redirect to PR1 pricing system for editing
        setLocation('/pr1-pricing');
      }
    }
    // Auto-redirect to PR1 pricing if autoFocus is present (for new pricing)
    else if (autoFocus) {
      console.log('Auto-focus detected, redirecting to PR1 pricing...');
      setLocation('/pr1-pricing');
    }
  ...
  useEffect(() => {
    ...
    if (autoFocus && workCategories && Array.isArray(workCategories) && workCategories.length > 0 && !(editMode === 'true' && editId)) {
      ...
      if (matchingCategory) {
        ...
        // Redirect to PR1 pricing system for auto-populated data
        setLocation('/pr1-pricing');
      }
    }
  ...
  // Removed old formData useEffect hooks - now using PR1 pricing system
  ...
  // Removed old formData reset function - now using PR1 pricing system
  ...
  const proceedWithEditDirectly = (item: any) => {
    ...
    // Navigate to PR1 pricing system for editing
    setLocation('/pr1-pricing');
  };
  ...
  const proceedWithEdit = () => {
    ...
    console.log("proceedWithEdit called, redirecting to PR1 pricing after compliance warning");
    
    // Navigate to PR1 pricing system for editing after compliance warning
    setLocation('/pr1-pricing');
  ...
  // Removed old pricing dialog logging
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {/* Old pricing dialog completely removed - now routing to PR1 pricing system */}
