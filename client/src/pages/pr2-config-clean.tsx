...
export default function PR2ConfigClean() {
  ...
  const getTemplateType = (categoryId: string): 'TP1' | 'TP3' | 'P26' | 'P006a' => {
    if (categoryId === 'robotic-cutting') {
      ...
      return 'TP1'; // All other categories use standard TP1 template (including cctv-jet-vac ID 175)
    }
  };
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (categoryId === 'patching' && payload.vehicleTravelRates?.length > 0) {
            console.log('🔄 P26 SYNC: Syncing vehicle travel rates across configurations...');
