        ...
        {categoryId === 'cctv-jet-vac' && (
          <div className="flex gap-6 mb-6">
            ...
            <CardHeader className="pb-2">
              <CardTitle className="text-cyan-600 font-semibold text-sm flex items-center justify-between">
                    ...
                    formData.vehicleTravelRates.map((vehicle, index) => (
                      <div key={vehicle.id} className="flex gap-2 items-center">
                        <Select
                          ...
                          onValueChange={(value) => {
                            const updatedVehicle = { ...vehicle, vehicleType: value };
                        ...
                        {index === 0 ? (
                          ...
                          <Button
                            ...
                            onClick={() => {
                              setFormData(prev => ({
                                ...prev,
                                vehicleTravelRates: prev.vehicleTravelRates.filter(v => v.id !== vehicle.id),
                                vehicleTravelRatesStackOrder: prev.vehicleTravelRatesStackOrder.filter(id => id !== vehicle.id)
                              }));
                              debouncedSave();
                            }}
                      ...
                      <div className="flex gap-2 items-center">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            setFormData(prev => ({
                              ...prev,
                              vehicleTravelRates: [newVehicle],
                              vehicleTravelRatesStackOrder: [newVehicle.id]
                            }));
                            debouncedSave();
                          }}
                          className="h-7 w-24 p-0 text-white hover:bg-green-200 bg-green-600 border border-green-700"
...
const TP1TemplateInterface: React.FC<TP1TemplateInterfaceProps> = ({ pipeSize, sector }) => {
  ...
  const addRangeOption = () => {
    ...
    const newRangeOption = {
      ...
      label: tp1Data.rangeOptions.length % 2 === 0 ? 'Percentage' : 'Length',
      enabled: true,
      rangeStart: '',
      rangeEnd: ''
    };
  ...
  const deletePairedRow = (index: number) => {
    setTp1Data(prev => {
      // Delete corresponding quantity option (green window)
      const newQuantityOptions = prev.quantityOptions.filter((_, i) => i !== index);
      const deletedQuantityId = prev.quantityOptions[index]?.id;
      
      // Delete range option (purple window)
      const newRangeOptions = prev.rangeOptions.filter((_, i) => i !== index);
      const deletedRangeId = prev.rangeOptions[index]?.id;
      
      return {
        ...prev,
        quantityOptions: newQuantityOptions,
        quantityStackOrder: (prev.quantityStackOrder || []).filter(id => id !== deletedQuantityId),
