...
© 2025 Professional Sewer Condition Assessment Services`;
  ...
  app.put('/api/equipment-types/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Handle standard equipment IDs (string format) vs database IDs (numeric)
      if (idParam.startsWith('standard-')) {
        // For standard equipment, we don't actually update the database
        // Standard equipment should be read-only or handled differently
        res.status(400).json({ message: "Standard equipment cannot be modified" });
        return;
      }
      ...
      console.log('Raw values - minPipeSize:', minPipeSize, 'maxPipeSize:', maxPipeSize, 'costPerDay:', costPerDay, 'workCategoryId:', workCategoryId);
