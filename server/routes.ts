...
© 2025 Professional Sewer Condition Assessment Services`;
  ...
  // User cost bands management
  app.get('/api/cost-bands/:sector', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const sector = req.params.sector;
      
      const userCostBands = await storage.getUserCostBands(userId, sector);
      
      // If no custom cost bands exist, return default ones
      if (userCostBands.length === 0) {
        const defaultCostBands = [
          { grade: 0, costBand: '£0', sector },
          { grade: 1, costBand: '£0-500', sector },
          { grade: 2, costBand: '£500-2,000', sector },
          { grade: 3, costBand: '£2,000-10,000', sector },
          { grade: 4, costBand: '£10,000-50,000', sector },
          { grade: 5, costBand: '£50,000+', sector }
        ];
        res.json(defaultCostBands);
      } else {
        res.json(userCostBands);
      }
    } catch (error) {
      console.error('Error fetching cost bands:', error);
      res.status(500).json({ message: 'Failed to fetch cost bands' });
    }
  });

  app.post('/api/cost-bands', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { grade, costBand, sector } = req.body;
      
      const newCostBand = await storage.createUserCostBand({
        userId,
        grade,
        costBand,
        sector,
      ...
      res.json(newCostBand);
    } catch (error) {
      console.error('Error creating cost band:', error);
      res.status(500).json({ message: 'Failed to create cost band' });
    }
  });

  app.put('/api/cost-bands/:id', isAuthenticated, async (req: any, res) => {
    try {
      const { id } = req.params;
      const { costBand } = req.body;
      
      const updatedCostBand = await storage.updateUserCostBand(parseInt(id), costBand);
      res.json(updatedCostBand);
    } catch (error) {
      console.error('Error updating cost band:', error);
      res.status(500).json({ message: 'Failed to update cost band' });
    }
  });

  app.delete('/api/cost-bands/:sector/reset', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const sector = req.params.sector;
      
      await storage.resetUserCostBands(userId, sector);
      res.json({ message: 'Cost bands reset to defaults' });
    } catch (error) {
      console.error('Error resetting cost bands:', error);
      res.status(500).json({ message: 'Failed to reset cost bands' });
    }
  });

  // Check if pricing exists for a sector
  app.get('/api/pricing/check/:sector', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { sector } = req.params;
      
      // Get all user pricing for different work categories
      const surveyPricing = await storage.getUserPricing(userId, 1); // Category 1: Surveys
      const cleansingPricing = await storage.getUserPricing(userId, 2); // Category 2: Cleansing
      const jettingPricing = await storage.getUserPricing(userId, 4); // Category 4: Directional Water Cutting
      
      const hasPricing = {
        surveys: surveyPricing.length > 0,
        cleansing: cleansingPricing.length > 0,
        jetting: jettingPricing.length > 0,
        overall: surveyPricing.length > 0 || cleansingPricing.length > 0 || jettingPricing.length > 0
      };
      
      res.json(hasPricing);
    } catch (error) {
      console.error('Error checking sector pricing:', error);
      res.status(500).json({ message: 'Failed to check pricing availability' });
    }
  });

  // Pricing Rules API endpoints
  app.get('/api/pricing-rules/:categoryId', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const categoryId = parseInt(req.params.categoryId);
      
      const rules = await storage.getUserPricingRules(userId, categoryId);
      res.json(rules);
    } catch (error) {
  ...
  app.post('/api/pricing-rules', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const ruleData = {
        ...req.body,
  ...
  app.put('/api/pricing-rules/:id', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.delete('/api/pricing-rules/:id', isAuthenticated, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
  ...
  // Standards-based recommendations API endpoints
  app.get('/api/standards-rules/:sector', isAuthenticated, async (req: any, res) => {
    try {
      const { sector } = req.params;
      const userId = req.user.claims.sub;
      const rules = await storage.getStandardsRules(userId, sector);
      res.json(rules);
    } catch (error) {
      console.error("Error fetching standards rules:", error);
      res.status(500).json({ message: "Failed to fetch standards rules" });
    }
  });

  app.post('/api/standards-rules', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const ruleData = { ...req.body, userId };
      const rule = await storage.createStandardsRule(ruleData);
      res.json(rule);
    } catch (error) {
      console.error("Error creating standards rule:", error);
      res.status(500).json({ message: "Failed to create standards rule" });
    }
  });

  app.put('/api/standards-rules/:id', isAuthenticated, async (req: any, res) => {
    try {
      const { id } = req.params;
      const userId = req.user.claims.sub;
      const rule = await storage.updateStandardsRule(parseInt(id), userId, req.body);
      res.json(rule);
    } catch (error) {
      console.error("Error updating standards rule:", error);
      res.status(500).json({ message: "Failed to update standards rule" });
    }
  });

  app.delete('/api/standards-rules/:id', isAuthenticated, async (req: any, res) => {
    try {
      const { id } = req.params;
      const userId = req.user.claims.sub;
      await storage.deleteStandardsRule(parseInt(id), userId);
      res.json({ message: "Standards rule deleted successfully" });
    } catch (error) {
      console.error("Error deleting standards rule:", error);
      res.status(500).json({ message: "Failed to delete standards rule" });
    }
  });

  // Condition-trigger-action API endpoints
  app.get('/api/condition-triggers', async (req, res) => {
    const { getAllConditions, getAllRecommendedActions, CONDITION_TRIGGER_ACTIONS } = await import('./condition-trigger-actions');
    res.json({
      conditions: getAllConditions(),
      recommendedActions: getAllRecommendedActions(),
      allMappings: CONDITION_TRIGGER_ACTIONS
    });
  });
  ...
  // New detailed pricing system routes
  app.get('/api/work-categories', isAuthenticated, async (req: any, res) => {
    try {
  ...
  app.get('/api/equipment-types/:categoryId', isAuthenticated, async (req: any, res) => {
    try {
      const categoryId = parseInt(req.params.categoryId);
      // Remove sector filtering to show all equipment
      const equipmentTypes = await storage.getEquipmentTypesByCategory(categoryId);
  ...
  // Sector-specific pricing rules endpoints
  app.get('/api/pricing-rules/:sector', isAuthenticated, async (req: any, res) => {
    try {
      const sector = req.params.sector;
      const userId = req.user.claims.sub;
      const pricingRules = await storage.getPricingRulesBySector(userId, sector);
  ...
  app.post('/api/pricing-rules', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const ruleData = { ...req.body, userId };
      const newRule = await storage.createPricingRule(ruleData);
      res.json(newRule);
    } catch (error) {
      console.error("Error creating pricing rule:", error);
      res.status(500).json({ message: "Failed to create pricing rule" });
  ...
  app.put('/api/pricing-rules/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const userId = req.user.claims.sub;
      const updatedRule = await storage.updatePricingRule(ruleId, userId, req.body);
  ...
  app.delete('/api/pricing-rules/:id', isAuthenticated, async (req: any, res) => {
    try {
      const ruleId = parseInt(req.params.id);
      const userId = req.user.claims.sub;
      await storage.deletePricingRule(ruleId, userId);
      res.json({ message: "Pricing rule deleted successfully" });
    } catch (error) {
      console.error("Error deleting pricing rule:", error);
      res.status(500).json({ message: "Failed to delete pricing rule" });
  ...
  app.get('/api/user-pricing', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const equipmentTypeId = req.query.equipmentTypeId ? parseInt(req.query.equipmentTypeId as string) : undefined;
      const userPricing = await storage.getUserPricing(userId, equipmentTypeId);
  ...
  app.post('/api/user-pricing', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      
      // Clean up numeric fields - convert empty strings to null
      const cleanedData = { ...req.body };
      const numericFields = ['costPerHour', 'costPerDay', 'meterageRangeMin', 'meterageRangeMax', 'sectionsPerDay'];
      ...
      const pricingData = { ...cleanedData, userId };
      const newPricing = await storage.createUserPricing(pricingData);
      res.json(newPricing);
    } catch (error) {
      console.error("Error creating user pricing:", error);
      res.status(500).json({ message: "Failed to create user pricing" });
    }
  });

  app.put('/api/user-pricing/:id', isAuthenticated, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
      ...
      const updatedPricing = await storage.updateUserPricing(id, cleanedData);
      res.json(updatedPricing);
    } catch (error) {
      console.error("Error updating user pricing:", error);
      res.status(500).json({ message: "Failed to update user pricing" });
    }
  });

  app.delete('/api/user-pricing/:id', isAuthenticated, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
      await storage.deleteUserPricing(id);
      res.json({ message: "User pricing deleted successfully" });
    } catch (error) {
      console.error("Error deleting user pricing:", error);
      res.status(500).json({ message: "Failed to delete user pricing" });
    }
  });
  ...
  app.post('/api/equipment-types', isAuthenticated, async (req: any, res) => {
    try {
      const { name, description, minPipeSize, maxPipeSize, category, sector, costPerDay } = req.body;
      
      const newEquipment = await storage.createEquipmentType({
  ...
  app.put('/api/equipment-types/:id', isAuthenticated, async (req: any, res) => {
    try {
      const idParam = req.params.id;
      const { name, description, minPipeSize, maxPipeSize, costPerDay, workCategoryId, sector, category } = req.body;
      
      // Only handle numeric database equipment IDs
      ...
      const parseNumber = (value: any, fallback: number) => {
        if (value === null || value === undefined || value === '' || value === 'NaN' || String(value) === 'NaN') {
          return fallback;
        }
      ...
      const sanitizedMinPipe = parseNumber(minPipeSize, 75);
      const sanitizedMaxPipe = parseNumber(maxPipeSize, 300);
      const sanitizedCost = parseNumber(costPerDay, 0);
      const sanitizedWorkCategoryId = parseNumber(workCategoryId, 1);
      
      const updateData: any = {
        name,
        description,
        minPipeSize: sanitizedMinPipe,
        maxPipeSize: sanitizedMaxPipe
      };
      
      // Only include costPerDay if it's provided and valid
      if (costPerDay !== undefined) {
        updateData.costPerDay = sanitizedCost;
      }
      
      // Only include workCategoryId if it's provided - use sanitized value
      if (workCategoryId !== undefined) {
        updateData.workCategoryId = sanitizedWorkCategoryId;
      }
      
      // Only include sector if it's provided
      if (sector !== undefined) {
      ...
      const updatedEquipment = await storage.updateEquipmentType(id, updateData);
  ...
  app.delete('/api/equipment-types/:id', isAuthenticated, async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
