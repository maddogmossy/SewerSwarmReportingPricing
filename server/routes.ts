...
© 2025 Professional Sewer Condition Assessment Services`;
  ...
  app.get('/api/equipment-types/:categoryId', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const equipmentTypes = await storage.getEquipmentTypesByCategory(categoryId, sector);
  ...
  app.post('/api/equipment-types', isAuthenticated, async (req: any, res) => {
    try {
      const { name, description, minPipeSize, maxPipeSize } = req.body;
  ...
  app.put('/api/equipment-types/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Prevent standard equipment from being saved to database
      if (typeof idParam === 'string' && idParam.startsWith('standard-')) {
        return res.status(200).json({
          success: true,
          message: "Standard equipment updated in memory only"
        });
      }
      
      // Validate and sanitize numeric fields - handle both number and string NaN
      ...
      // Handle standard equipment IDs (string format) - convert to database operations
      if (idParam.startsWith('standard-')) {
        // For standard equipment, create a new database entry with the updated values
        // This allows users to "modify" standard equipment by creating personalized versions
        ...
        // Use provided category or determine from standard equipment ID
        let equipmentCategory = category || 'CCTV'; // Use provided category first
        if (!category) {
          // Fallback to ID-based logic for standard equipment
          if (idParam.includes('7') || idParam.includes('8') || idParam.includes('9')) {
            equipmentCategory = 'Jetting';
          } else if (idParam.includes('10') || idParam.includes('11')) {
            equipmentCategory = 'Patching';
          }
        }
        ...
        const newEquipment = {
          ...
          costPerDay: sanitizedCost,
          workCategoryId: sanitizedWorkCategoryId,
          sector: sector || 'utilities',
          category: equipmentCategory
        };
        
        const created = await storage.createEquipmentType(newEquipment);
        res.json(created);
        return;
      }
