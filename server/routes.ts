    ...
    try {
      ...
      async function parsePDFInspectionReport(filePath: string) {
        try {
          ...
          // Read and parse the actual PDF file
      ...
      // This function now processes the sections returned from PDF parsing
      async function processPDFSections(inspectionSections: any[]) {
        try {
          const sections = [];
...
© 2025 Professional Sewer Condition Assessment Services`;
  ...
  // Sewer Cleaning Manual API endpoint
  app.get('/api/sewer-cleaning/:defectCode', isAuthenticated, async (req: any, res) => {
    try {
      const { defectCode } = req.params;
      const { SEWER_CLEANING_MANUAL } = await import('./sewer-cleaning');
      
      const cleaningData = SEWER_CLEANING_MANUAL[defectCode];
      
      if (!cleaningData) {
        return res.status(404).json({ 
          message: `No cleaning recommendations found for defect code: ${defectCode}` 
        });
      }
      
      res.json(cleaningData);
    } catch (error) {
      console.error('Error fetching sewer cleaning data:', error);
  ...
  app.post('/api/pricing-rules', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const ruleData = {
        ...req.body,
        userId,
        workCategoryId: req.body.workCategoryId || 1 // Default to category 1 (Surveys) for sector-based rules
      };
