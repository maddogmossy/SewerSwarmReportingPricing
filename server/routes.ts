...
© 2025 Professional Sewer Condition Assessment Services`;
  ...
  app.put('/api/equipment-types/:id', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const sanitizedMinPipe = typeof minPipeSize === 'number' && !isNaN(minPipeSize) ? minPipeSize : 75;
      const sanitizedMaxPipe = typeof maxPipeSize === 'number' && !isNaN(maxPipeSize) ? maxPipeSize : 300;
      const sanitizedCost = typeof costPerDay === 'number' && !isNaN(costPerDay) ? costPerDay : 0;
