...
// New function: Process authentic Wincan database with Meta.db3 support via ATTACH DATABASE
export async function readWincanDatabase(
  mainDbPath: string,
  ...
  if (metaDbPath) {
    const attachQuery = `ATTACH DATABASE '${metaDbPath}' AS metaDb;`;
    await new Promise((resolve, reject) => {
      db.run(attachQuery, (err: any) => {
        if (err) {
          console.error("❌ Failed to attach Meta.db3:", err.message);
          return reject(err);
        }
        console.log("📎 Meta.db3 attached:", metaDbPath);
        resolve(true);
      });
    });
  }

  const sector = options?.sector || 'utilities';
  ...
  return new Promise((resolve, reject) => {
    db.all(query, [], async (err: any, sectionRecords: any[]) => {
      ...
      db.all(observationQuery, [], async (obsErr: any, observationRecords: any[]) => {
        ...
        // Get authentic severity grades from SECSTAT table (main DB first, then Meta if available)
        let secstatQuery = `SELECT * FROM SECSTAT`;
        if (metaDbPath) {
          secstatQuery = `
            SELECT * FROM SECSTAT
            UNION ALL
            SELECT * FROM metaDb.SECSTAT
          `;
        }
        
        db.all(secstatQuery, [], async (secErr: any, secstatRecords: any[]) => {
          if (secErr) {
            console.warn('⚠️ No SECSTAT data found:', secErr.message);
            secstatRecords = [];
          }
          ...
          const severityMap: { [sectionRef: string]: { structural: number, service: number } } = {};
          for (const secRecord of secstatRecords) {
            // Map SECSTAT records to their respective sections
            const sectionRef = secRecord.STA_Section_REF || secRecord.Section_REF;
            if (sectionRef) {
              const structural = secRecord.STA_StructGrade || secRecord.StructuralGrade || 0;
              const service = secRecord.STA_ServGrade || secRecord.ServiceGrade || 0;
          ...
          for (const record of sectionRecords) {
            ...
            // Extract pipe size with proper numeric handling
            const rawPipeSize = extractAuthenticValue(record, ['OBJ_Size1', 'Size1', 'Diameter']);
            const pipeSize = rawPipeSize ? Math.round(parseFloat(rawPipeSize)) : 0;
            
            // Extract pipe material
            const pipeMaterial = extractAuthenticValue(record, ['OBJ_Material', 'Material']) || 'Unknown';
            
            // Extract length with proper numeric handling
            const rawLength = extractAuthenticValue(record, ['OBJ_Length', 'Length', 'TotalLength']);
            const totalLength = rawLength ? parseFloat(rawLength) : 0;
            
            // Extract date and time information
            const rawTimestamp = extractAuthenticValue(record, ['OBJ_TimeStamp', 'TimeStamp', 'Date']);
            const inspectionDate = rawTimestamp ? new Date(rawTimestamp).toISOString().split('T')[0] : new Date().toISOString().split('T')[0];
            const inspectionTime = rawTimestamp ? new Date(rawTimestamp).toISOString().split('T')[1]?.split('.')[0] || '00:00:00' : '00:00:00';
            
            // Get observations for this section
            const sectionRef = record.OBJ_PK || record.PK;
            const observations = observationMap[sectionRef] || [];
            
            // Format observations with authentic remark enhancement
            let defectText = '';
            if (observations.length > 0) {
              const enhancedObservations = observations.map(obs => enhanceObservationWithRemark(obs));
              defectText = await formatObservationText(enhancedObservations, sector);
            } else {
            ...
            const sectionData: WincanSectionData = {
              itemNo: authenticItemNo,
              projectNo: record.OBJ_Name || options?.originalFileName?.split(' ')[0] || 'Unknown',
              startMH: startMH,
              finishMH: finishMH,
              pipeSize: pipeSize.toString(),
              pipeMaterial: pipeMaterial,
